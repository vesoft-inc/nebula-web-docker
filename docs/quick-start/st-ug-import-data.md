...
7. 在 **vertexId** 部分，完成以下操作：
   a. 在 **对应列标** 列，点击 **选择**。  
   ![在数据源中点击“选择”](/docs/figs/st-ug-009.png "为 vertexId 选择数据源")
...
8. 在 **TAG 1** 部分，完成以下操作：
   ...
   ![course类点对应的属性数据源](/docs/figs/st-ug-010.png "为点属性选择数据源")
...
![actions 边对应的属性数据源](/docs/figs/st-ug-011.png "为边属性选择数据源")
14. 完成设置后，点击 **下一步** 按钮。
15. 在 **导入** 页面，点击 **导入** 按钮开始导入数据。在 **log** 页面上，您可以看到数据导入进度。导入所需时间因数据量而异。导入过程中，您可以点击 **终止导入** 停止数据导入。当 **log** 页面显示如图所示信息时，表示数据导入完成。  
![log 里最后显示导入操作完成的时间、导入的行数以及操作操作的行数](/docs/figs/st-ug-005.png "导入结束时的 log 信息")

## 后续操作

完成数据导入后，您可以开始 [图探索](docs/quick-start/st-ug-explore.md)。
